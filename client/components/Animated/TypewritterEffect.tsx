...
import { countMatchingCharacters, delay, repeatFunctionNTimes } from "./utils";
                ...
                if (i >= textArray.length) {
                    ...
                    if (direction === "front") {
                        ...
                        setText(
                            (currText) =>
                                `${
                                    textArray[textArray.length - i - 1]
                                }${currText}`,
                        );
                    }
                }
            ...
            const interval = setInterval(() => {
                if (i >= count) {
                    ...
                    if (direction === "front") {
                        setText(
                            (currtext) =>
                                `${currtext.substring(0, currtext.length - 1)}`,
                        );
                    } else {
                        setText(
                            (currtext) =>
                                `${currtext.substring(1, currtext.length)}`,
                        );
                    }
                }
                i++;
        ...
        for (const data of sequence) {
            if (data?.action) {
                ...
                if (data?.delayBetweenSequence) {
                    await delay(
                        data?.delayBetweenSequence ?? delayBetweenSequence,
                    );
                }
            } else if (currentText) {
